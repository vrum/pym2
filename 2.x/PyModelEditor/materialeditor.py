...
class MaterialEditor(QtGui.QDialog):
	...
	def setModel(self,m2):
		self.m2 = m2
		for i in range(len(self.m2.materials[0])):
			self.chooseBox.addItem(str(i))
		if len(self.m2.materials[0]) != 0:
			self.lastchoice = 0

		for i in range(len(self.m2.submeshes[0])):
			temp = str(self.m2.submeshes[0][i].mesh_id)
			j = len(temp)
			if (j<3):				
				number = temp
				temp = "00"
			...
			try:
				...
				print temp
				print self.m2.submeshes[0][i].mesh_id
				self.submeshChoose.addItem(str(i)+":"+str(self.m2.submeshes[0][i].mesh_id))
	...
	def changeEdit(self):
		...
		tu = self.m2.materials[0][self.chooseBox.currentIndex()]
		if tu.flags & 0x10:
	...
	def saveOld(self):
		...
		tu = self.m2.materials[0][self.lastchoice]
		...
		self.m2.materials[0][self.lastchoice] = tu
